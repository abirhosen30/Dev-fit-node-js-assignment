...
const ExercisePlanCard = ({ exercise, removeFromTodayPlan }: ExercisePlanCardProps) => {
  return (
    <div className="flex items-center mb-4 gap-4 border-b border-[#252a33] bg-[#0f1218] px-4 py-3">
      {" "}
      {/* IMAGE */}{" "}
      ...
      <div className="flex shrink-0 items-center gap-2">
        {" "}
       <Link href={`/details/${exercise.id}`} className="flex items-center gap-2">
        <button
          type="button"
        ...
        {<button
        onClick={() => removeFromTodayPlan(exercise.id)}
          type="button"
